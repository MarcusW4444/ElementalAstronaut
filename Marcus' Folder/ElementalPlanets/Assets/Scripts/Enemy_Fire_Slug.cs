...
public class Enemy_Fire_Slug : GenericEnemy {
    ...
    void FixedUpdate()
    {
                        ...
                        else if (rh.distance <= .6f)
                        {
                            ...
                            if (!isStunned())
                                //if (Mathf.Abs(sdif) <= 3f)
                                    MyRigidbody.velocity = new Vector2((GoLeftGoRight ? -1f : 1f)*(cancrawl? 1f:0f) * 2f*((.5f + (2f * Astronaut.AggressionLevelF))),MyRigidbody.velocity.y);
                        }
        ...
        if (Alive)
        {
            ...
            if (((!IsInFirePatch) && ((Time.time - LastFirePatchDroppedTime)>= .8f))&&(!isStunned()))
            {
                spawnFirePatch();
            }
