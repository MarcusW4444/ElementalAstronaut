...
Shader "Custom/CamShader" {
	...
	Properties{
		...
		_QuellColor("Quell Color",Color) = (1,1,1,1)
		_QuellValue("Quell Value",Range(0,1)) = 1
...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : SV_Target
	{
		...
		float rs = sqrt(pow((i.grabPos.x * 2) - 1, 2) + pow((i.grabPos.y*2) - 1,2))/(sqrt(2)); //Distance from the center 0->1->sqrt(2)
		...
		if ((_Death > 0)&& (rs > _LifeValue) && (_LifeValue < 1)) {
			...
			half ru = (rs / sqrt(2));
			if (_QuellValue >= ru) {
				half4 invcolor = half4(1.0-bgcolor.r, 1.0 - bgcolor.g, 1.0 - bgcolor.b,bgcolor.a);
				half me = max(max(invcolor.r, invcolor.g), invcolor.b);
				float tu = 1;
				bgcolor = lerp(half4(m*_QuellColor.r, m*_QuellColor.g, m*_QuellColor.b, 1), half4(me*_QuellColor.r, me*_QuellColor.g, me*_QuellColor.b, 1),max(0,ru-(1-tu))/tu);//Tint the Quell Color if it is within the threshold
			}
