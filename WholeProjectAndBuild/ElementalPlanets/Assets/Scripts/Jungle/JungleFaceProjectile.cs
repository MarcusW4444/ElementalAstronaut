...
public class JungleFaceProjectile : EnemyProjectile
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if ((plr != null) && (plr.Alive) && (!plr.Invulnerable) && (Live) && ((Time.time - plr.lastDamageTakenTime) >= 1f))
        {
            ...
            if (plr.Alive)
            {
                plr.affectSpore(4f * Astronaut.AggressionLevelF);
            }
